...
@Suite struct SQLiteTests {
    ...
    @Test func iterateTextRows() throws {
        ...
        let statement = try Statement(sql, connection: connection)
        // compile statement
        let results = try statement.withColumns(connection) { (columns: Statement.Column) throws(SQLiteError) -> [String: String] in
            var object = [String: String]()
            for (index, column) in columns.enumerated() {
                let text = try statement.handle.readText(at: Int32(index), connection: connection.handle).get()
                object[column] = text
            }
            return object
        }
