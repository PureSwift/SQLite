...
// `ISO8601DateFormatter` (an `NSFormatter` subclass) is only available with the full
// `Foundation` module — `FoundationEssentials` only ships the newer `ISO8601Format()`
// format-style API. Prefer the lean module where it's sufficient on its own.
